...
class HttpRoutesTest {
    ...
    private val todoListDetailsQueryHandler = mockk<TodoListDetailsQueryHandler>()
    ...
    fun `should get a todo-list details`(): Unit = withTestApp {
        ...
        every {
            todoListDetailsQueryHandler.get(id)
        } returns todoListDetailDto
    ...
    private fun withTestApp(callback: TestApplicationEngine.() -> Unit): Unit {
        withTestApplication({
            ...
            install(Routing) {
                todoListDetails(todoListDetailsQueryHandler)
            }
        }, callback)
    }
